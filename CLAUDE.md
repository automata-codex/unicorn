...
- **A baseline run is not closed until `docs/eval-methodology.md § Current baseline N` names it.** That section carries one standing-point statement; accepting a new baseline means *replacing* it, not appending beside it. A run that happened and was not accepted still needs saying so there — the failure mode is a run nobody dispositioned, which is how that section fell four runs behind by 2026-08-24. This is the write-up's job, not the run's: the run is the maintainer's, the record is whoever reads the artifacts.
