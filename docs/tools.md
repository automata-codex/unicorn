...
const submitGmResponseSchema = z.object({
  ...
  stateChanges: z.object({
    ...
    // Example: { "dr_chen_stress": { "delta": 2 } }
    resourcePools: z.record(
      z.string(),
      z.object({ delta: z.number().int() })
    ).optional(),

    // HP changes and position updates for entities.
    entities: z.record(
      z.string(),
      z.object({
        hp:       z.object({ delta: z.number().int() }).optional(),
        position: z.object({
          x: z.number().int(),
          y: z.number().int(),
  ...
  diceRequests: z.array(z.object({
    id:       z.string().uuid(),   // generated by Claude; referenced in the player's diceResult submission
...
**Validation behavior:**
- `resourcePools` deltas that would reduce a pool below zero are rejected. The backend notifies Claude, which re-narrates the consequence.
- `entities.hp` negative deltas that would reduce HP to zero or below trigger death/unconscious threshold checks. The backend applies the threshold and notifies Claude of the actual outcome.
...
Claude API call initiated
  ...
  └─ Claude calls submit_gm_response
       ...
       → player_text and diceRequests returned to frontend
