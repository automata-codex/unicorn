...
milestone: unknown
summary: >-
  Two artifacts that were originally both called `adventure_log`, kept apart
  deliberately: `adventure_telemetry` is per-turn diagnostic infrastructure and not
  player-facing, while the session export is the on-demand portable file supporting
  restore and post-session analysis.
---
