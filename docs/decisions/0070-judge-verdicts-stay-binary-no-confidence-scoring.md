...
milestone: unknown
summary: >-
  Judge verdicts stay `{passed, rationale}`. Records a decision that predates the
  multi-run harness and had lived only in the shape of a schema, because a
  permanently-empty optional confidence column reads as an invitation to fill it
  rather than as a decision. Adding one later is non-breaking.
---
