...
- [x] Chunking iteration against `task eval:retrieval` — one change per round, each logged in `docs/rules-extraction-findings.md`, including the rounds that made things worse. Closes on the bar or on the stopping rule, whichever comes first. **Three rounds run (`§ S17`, `§ S18`, `§ S19`); closed on the stopping rule with two of three bar metrics met** — `authored` held at 100.0%, MRR 0.856 against a 0.85 bar, `warden-observed` recall@3 91.3% against 95.6%. The shortfall is 21 of 23 where the bar asks 22, and both misses are now diagnosed rather than open
