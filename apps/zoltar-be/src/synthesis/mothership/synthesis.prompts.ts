...
export function buildMothershipSynthesisPrompt(
  ...
  const sections = [
    ...
    `WORLD FACTS:\nUse structured.worldFacts for any non-numeric initial state the Warden needs to remember across turns — starting deck or location name, environmental detail that must stay consistent (specific graffiti text, console readout content), NPC cover identities, etc. Keys should be descriptive snake_case. Values are plain strings. Do not put numeric state here — use initialState for resource pools and countdown timers.`,
